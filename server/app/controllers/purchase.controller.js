  ...
  exports.getOrders = (req, res) => {
    ...
    exports.getOrder = (req, res) => {
       ...
       console.log(id);
        ...
        //let query = "SELECT Status_code FROM `status` JOIN  `order_items` ON order_items.item_id = order_items.item_id JOIN `orders` ON status.Status_code= orders.status WHERE user.id = " + id;
        let query = "SELECT Status_code FROM `status` JOIN `orders` ON orders.status = status.id JOIN `order_items` ON orders.id = order_items.order_id WHERE orders.id = 2";
